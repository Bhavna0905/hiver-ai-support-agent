...
The strongest properties are safety and style, while resolution correctness remains the main weakness. The agent generally produces safe, relevant responses grounded in historical examples, but it does not always transfer the exact historical resolution to the current customer's situation.
...
Agreement is moderate overall but weak for several individual dimensions. Therefore, LLM-judge scores are treated as a secondary evaluation signal rather than ground truth. The calibration used one human reviewer.
...
**Potential improvement:** detect unsupported/malformed input and route it to `other` or human review.
...
**Hypothesis:** historical retrieval provides evidence of similar resolutions, but a 3B local model can fail to correctly transfer that resolution to the current situation.
...
- Calibrate MiniLM confidence and add a second-stage classifier for ambiguous intent pairs.
- Improve malformed/low-context message detection and route uncertain cases to `other` or human review.
- Add structured extraction of the intended resolution from retrieved historical examples before generation.
- Add a post-generation check for resolution completeness and contradictions with the retrieved evidence.
- Expand human evaluation beyond the current 20-example judge-calibration subset.